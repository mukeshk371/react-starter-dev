  ...
  return (
    <div className="p-[24px]">
      <Link to="/" className="btn btn-primary mb-3 d-inline-flex align-items-center">
        <ArrowLeft className="me-2" />Back to E-commerce Page
      </Link>
      <h2>Cart Items</h2>
          ...
          {cartItems.map((item, index) => (
            <tr key={index}>
              ...
              <td className="text-center border border-black md:p-[10px]">
                <button className="bg-red-600 p-[6px] text-white rounded" onClick={() => removeFromCart(index)}>
                  <DashLg />
                </button>
                <button className="bg-green-600 p-[6px] text-white rounded ml-[4px]" onClick={() => addToCart(index)}>
                  <PlusLg />
                </button>
              </td>
            </tr>
